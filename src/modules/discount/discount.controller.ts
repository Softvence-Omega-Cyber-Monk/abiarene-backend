...
export class DiscountController {
  ...
  create(@CurrentUser() user: AuthUser | undefined, @Body() dto: CreateDiscountDto) {
    return this.service.create(this.tenantId(user), dto, user?.role);
  }

  @Get()
  @Roles('manager', 'supervisor', 'owner', 'server', 'kitchen', 'cashier', 'admin')
  ...
  update(@CurrentUser() user: AuthUser | undefined, @Param('id') id: string, @Body() dto: UpdateDiscountDto) {
    return this.service.update(this.tenantId(user), id, dto, user?.role);
  }

  @Delete(':id')
  @Roles('supervisor', 'owner', 'admin')
